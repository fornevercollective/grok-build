...
pub(crate) async fn run(
    ...
    // Media launch (`launch-watch.sh` / `launch-webgrid.sh` / `launch-timesync.sh` / `launch-map.sh`,
    // `GROK_LIVE_WATCH` / `GROK_OPEN_TIMESYNC` / `GROK_MAP_TARGET`,
    // `GROK_NEW_SESSION_AT_STARTUP=1`): skip the welcome menu and land on an
    // agent prompt (and optionally open `/watch` `/webgrid` `/timesync` `/map` immediately).
    {
