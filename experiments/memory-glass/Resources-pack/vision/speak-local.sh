...
PY
# Local Mini speaker — skip when phone is live so we don't layer Mini+phone audio
LIVE_STAMP="${GY_VISION_DIR:-$HOME/.panda/vision}/live.stamp"
PHONE_LIVE=0
if [[ -f "$LIVE_STAMP" ]]; then
  AGE=$(python3 -c "import time; print(time.time()-float(open('$LIVE_STAMP').read().strip() or 0))" 2>/dev/null || echo 999)
  # if still-pipe frame < 45s, phone cam is live → phone TTS only
  awk -v a="$AGE" 'BEGIN{exit !(a+0 < 45)}' && PHONE_LIVE=1 || PHONE_LIVE=0
fi
if [[ "${MG_SAY_LOCAL:-}" == "0" ]]; then
  PHONE_LIVE=1
fi
if [[ -n "$CLEAN" && "$PHONE_LIVE" -eq 0 ]]; then
  say -v "${GY_SAY_VOICE:-Samantha}" "$CLEAN" 2>/dev/null || say "$CLEAN"
elif [[ -n "$CLEAN" && "$PHONE_LIVE" -eq 1 ]]; then
  echo "SPOKE_LOCAL phone-loud-only (skip Mini say · live ${AGE:-?}s)"
fi
echo "SPOKE_LOCAL $(date -u +%H:%M:%SZ) ${#CLEAN}c snap=$SNAP tts=$TTS phone_live=$PHONE_LIVE"
