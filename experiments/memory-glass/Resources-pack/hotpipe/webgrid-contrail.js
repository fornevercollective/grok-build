...
(function () {
  ...
  var VER = "webgrid-contrail-v10-live";
  ...
  var overlay = null;
  var flowPanel = null;
  var showOverlay = true; /* v32: ON during play — user expects visible trails */
  var showFlow = true; /* v32: flow panel visible with lab kit */
  var Ngrid = 30;
  var stats = {
    samples: 0,
    strokes: 0,
  ...
  function ensureOverlay() {
    ...
    overlay.style.cssText =
      "position:fixed;inset:0;z-index:2147483646;pointer-events:none;width:100%;height:100%;opacity:1;visibility:visible";
  ...
  function draw() {
    ...
    for (var i = start; i < path.length; i++) {
      ...
      /* agent trails full brightness (was 0.45 → looked "no contrails") */
      if (a1.src === "agent" || a0.src === "agent") alpha = Math.min(1, alpha * 1.05);
      ctx.strokeStyle = trajColor(
        a1.traj || a0.traj || "cruise",
        alpha,
        a1.strain != null ? a1.strain : lastStrain
      );
      ctx.lineWidth = (a1.src === "agent" ? 2.8 : 2.4) + age * 2.2;
      ctx.lineCap = "round";
      ctx.beginPath();
