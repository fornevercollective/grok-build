 ...
 * VER: body-pose-v2-cover-align
 * v2: IK joints map through live PIP cover measure (no FOV norm-space boost —
 *     that made the skeleton slip off the body when phone was wide / user angled).
 */
(function () {
  "use strict";
  var VER = "body-pose-v2-cover-align";
  ...
  function lmToPx(nx, ny, W, H) {
    ...
    try {
      if (typeof window.__mgMapCover === "function") {
        ...
        return { x: m.x, y: m.y, cropX: m.cropX, cropY: m.cropY };
      }
    } catch (eM) {}
  ...
  /**
   * Alignment-first rig: keep s≈1 so joints stick to the cover-cropped PIP.
   * Old FOV boost (up to 1.55× around hip) pulled limbs off the body when the
   * phone still was wide and the inspect window only showed a center crop.
   * Optional tiny boost only when body is a pin-dot (<6% shoulder span).
   */
  function bodyRig(lm) {
    var ik = buildIk(lm);
    ...
    var s = 1;
    if (uiGet("bodyRigAuto", true) && span > 0.015 && span < 0.06) {
      s = Math.min(1.12, 0.08 / span);
    }
    var manual = uiGet("bodyRigScale", 1);
    if (manual && manual !== 1) s *= manual;
    /* hard cap — never expand enough to slip off cover crop */
    s = Math.max(0.95, Math.min(1.12, s));
    return { s: s, pcx: root.x, pcy: root.y, ik: ik, span: span };
  }

  function HP_pt(lm, i, rig, W, H) {
    var p = lm[i];
    if (!p || vis(p) < 0.22) return null;
    /* s≈1 identity for normal poses — map true landmark through cover */
    var nx = p.x;
    var ny = p.y;
    if (rig && rig.s && Math.abs(rig.s - 1) > 0.01) {
      nx = rig.pcx + (p.x - rig.pcx) * rig.s;
      ny = rig.pcy + (p.y - rig.pcy) * rig.s;
    }
  ...
  function drawBody(ctx, W, H) {
    ...
    /* angle readouts near elbows/knees — same map as joints */
    if (ik && ik.angles && uiGet("showBodyAngles", true) !== false) {
      ...
      function angLabel(idx, key) {
        var ang = ik.angles[key];
        if (ang == null) return;
        var p = HP_pt(lm, idx, rig, W, H);
        if (!p) return;
    ...
    try {
      if (typeof window.__mgMeasurePip === "function") {
        ...
        if (cm && (cm.cropX > 0.04 || cm.cropY > 0.04)) {
          cropNote =
            " · crop " +
            Math.round((cm.cropX || 0) * 100) +
            "/" +
            Math.round((cm.cropY || 0) * 100);
        }
      }
    } catch (eC) {}
    ctx.fillText(
  ...
  window.__mgOnStillFrame = function (img) {
    ...
    try {
      if (window.__mgLens && window.__mgLens.setSourceSize) {
        window.__mgLens.setSourceSize(state._srcW, state._srcH);
      }
    } catch (eLs) {}
    if (uiGet("showBody", true) === false) return;
    if (!mpPose) {
