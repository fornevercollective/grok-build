 ...
 * VER: mg-tools-drawer-v33-one-piano
 * Keys = ONE beats staff+piano + maze/gsplat/contrail (no triple keyboard).
 * Staff = catalogue lab (+ optional beats if user opens Staff).
 ...
 * Dual-open with DATA: equal stack, scrims don't kill the other side.
 * v30: full kbatch site map via __mgKbatchSite
 * v31: DEPLOY / Status (built-in page + Grok Build Lab fallback)
 */
(function () {
  "use strict";
  var VER = "mg-tools-drawer-v33-one-piano";
  ...
  function ensureCss() {
    ...
    st.textContent = [
      ...
      /* Dual-open with DATA: same stack, scrims don't kill the other side */
      ...
      "html.mg-left-open.mg-right-open #mg-tools-drawer.open,",
      "html.mg-left-open.mg-right-open #mg-right-drawer.open{",
      "  z-index:2147483645!important}",
      "html.mg-left-open #mg-top-right,html.mg-left-open #mg-mode-menu,",
      "html.mg-left-open #mg-dev{",
      "  z-index:2147483615!important}",
      "html.mg-left-open #mg-top-right{pointer-events:none!important}",
      ...
      "  background:rgba(50,52,60,0.98)!important;color:#fff!important}",
      /* drawer above canvas; scrim NEVER captures during play (blocked maze/board/contrail) */
      "html.mg-webgrid-play > #mg-tools-drawer,html > #mg-tools-drawer{",
      "  z-index:2147483635!important;pointer-events:auto!important;visibility:visible!important}",
      "html.mg-webgrid-play > #mg-tools-scrim,html > #mg-tools-scrim,",
      "html.mg-webgrid-play #mg-tools-scrim,html.mg-webgrid-playing #mg-tools-scrim,",
      "html.mg-webgrid-play-busy #mg-tools-scrim{",
      "  pointer-events:none!important;opacity:0!important;visibility:hidden!important}",
      "html > #mg-tools-scrim:not(.on){pointer-events:none!important;opacity:0!important}",
      ...
      "#mg-tools-scrim{",
      "  position:fixed!important;inset:0!important;z-index:2147483630;pointer-events:none!important;",
      "  background:rgba(0,0,0,0.22);opacity:0;transition:opacity .2s}",
      /* only when drawer open AND not mid-WebGrid agent play */
      "#mg-tools-scrim.on{opacity:1;pointer-events:auto}",
      "html.mg-webgrid-playing #mg-tools-scrim,",
      "html.mg-webgrid-play-busy #mg-tools-scrim,",
      "html.mg-webgrid-playing #mg-tools-scrim.on,",
      "html.mg-webgrid-play-busy #mg-tools-scrim.on{",
   ...
   * Must use setProperty(...,"important") — mount cssText left:0!important
   * was winning and left the rail floating mid-page over content. */
  function positionModeRail() {
    if (!modeRail) return;
    try {
      modeRail.style.setProperty("position", "fixed", "important");
      modeRail.style.setProperty("top", "50%", "important");
      modeRail.style.setProperty("transform", "translateY(-50%)", "important");
      modeRail.style.setProperty("right", "auto", "important");
      modeRail.style.setProperty("bottom", "auto", "important");
      modeRail.style.setProperty("z-index", "2147483647", "important");
      if (!open) {
        modeRail.style.setProperty("left", "0px", "important");
        try {
          document.documentElement.style.removeProperty("--mg-tools-w");
        } catch (e0) {}
        return;
      }
      var w = 340;
      ...
      if (mode === "keys") w = Math.max(w, Math.min(480, window.innerWidth * 0.94));
      /* expose for CSS siblings (rail is not a child of drawer) */
      try {
      ...
      modeRail.style.setProperty("left", Math.round(w) + "px", "important");
    } catch (e) {
      try {
        modeRail.style.setProperty("left", open ? "340px" : "0px", "important");
      } catch (e2) {}
    }
  }
      ...
      collapsible("kbatch", "kbatch · R4-data", false, function (box) {
        ...
        r.appendChild(
          act(
            ...
            "muted",
            function () {
              if (window.__mgKbatchSite) window.__mgKbatchSite.open("dojo");
              else if (window.__mgKbatchFleet) window.__mgKbatchFleet.openDojo();
        ...
        r2.appendChild(
          act(
            ...
            "hot",
            function () {
              if (window.__mgKbatchSite && window.__mgKbatchSite.openMap) {
                window.__mgKbatchSite.openMap();
                setStatus(window.__mgKbatchSite.report());
              } else {
                nav("https://kbatch.ugrad.ai/");
                setStatus("kbatch site module missing · opened live");
              }
            },
            { ico: "▣", sub: "All surfaces", wide: true, keepStatus: true }
          )
        );
        r2.appendChild(
          act(
            ...
            "ok",
            function () {
              if (window.__mgKbatchSite) {
                window.__mgKbatchSite.probeLocal().then(function (ok) {
                  if (ok) window.__mgKbatchSite.nav(window.__mgKbatchSite.local + "/");
                  else nav("https://kbatch.ugrad.ai/");
                  setStatus(ok ? "kbatch LOCAL :8899" : "local down · LIVE");
                });
              } else nav("http://127.0.0.1:8899/");
            },
            { ico: "⌂", sub: ":8899 mirror" }
          )
        );
        r2.appendChild(
          act(
            ...
            "primary",
            function () {
              if (window.__mgKbatchSite) window.__mgKbatchSite.open("for-ai");
              else nav("https://kbatch.ugrad.ai/for-ai.html");
            },
            { ico: "◈", sub: "Agent funnel" }
          )
        );
        r2.appendChild(
          act(
            ...
            "muted",
            function () {
              if (window.__mgKbatchSite) window.__mgKbatchSite.open("lyrics");
              else nav("https://kbatch.ugrad.ai/lyrics.html");
            },
            { ico: "♪", sub: "Charts" }
          )
        );
        ...
        r.appendChild(
          act(
            ...
            function () {
              ...
              if (A && A.toggle) {
                A.toggle();
                setStatus(A.report ? A.report() : "DRAW toggled");
              } else if (A && A.activate) {
                A.activate();
                setStatus(A.report ? A.report() : "DRAW on");
              } else setStatus("DRAW missing — ⌘⇧R (mg-site-annotate)");
            },
            { ico: "✎", sub: "annotate site · live Grok", keepStatus: true }
          )
        );
        r.appendChild(
          act(
      ...
      collapsible("shell", "Shell · map · portal", true, function (box) {
        ...
        r2.appendChild(
          act(
            ...
            function () {
              function go() {
                if (window.__mgDeploy) {
                  window.__mgDeploy.open();
                  setStatus(
                    window.__mgDeploy.report
                      ? "deploy · " + (window.__mgDeploy.ver || "ok")
                      : "deploy panel"
                  );
                  return;
                }
                try {
                  ...
                  setStatus("deploy page");
                } catch (e) {
                  nav("https://fornevercollective.github.io/grok-build/");
                }
              }
              if (window.__mgLazy && window.__mgLazy.need) {
                window.__mgLazy.need("deploy", function () {
                  go();
                });
              } else go();
            },
            { ico: "◈", sub: "Status · probes · lab fallback" }
          )
        );
        r2.appendChild(
          act(
            ...
            "primary",
            function () {
              if (window.__mgDeploy && window.__mgDeploy.openPage) {
                window.__mgDeploy.openPage();
                setStatus("deploy.html");
              } else if (window.ipc) {
                window.ipc.postMessage(
                  JSON.stringify({ op: "open_deploy", t: Date.now() })
                );
        ...
        r2.appendChild(
          act(
            ...
            "ok",
            function () {
              if (window.__mgDeploy && window.__mgDeploy.openLab) {
                window.__mgDeploy.openLab("git");
              } else {
                nav(
                  "file:///Volumes/qbitOS/00.dev/projects/grok-build/docs/architecture-lab/index.html"
                );
              }
              setStatus("architecture-lab");
            },
            { ico: "↗", sub: "git · architecture-lab" }
          )
        );
        r2.appendChild(
          act(
            ...
            function () {
              if (window.__mgDeploy && window.__mgDeploy.openLab) {
                window.__mgDeploy.openLab("pages");
              } else {
                nav("https://fornevercollective.github.io/grok-build/");
              }
              setStatus("Pages lab");
            },
            { ico: "☁", sub: "fornevercollective.github.io" }
          )
        );
        box.appendChild(r2);
      })
  ...
  function paintKeys(into) {
    ...
    /* ONE piano only: keyboard-beats (staff + piano strip).
     * Do not also embed float-keyboard (QWERTY) or staff-lab — that made 3 pianos. */
    var stack = document.createElement("div");
    stack.className = "mg-keys-stack";
    stack.setAttribute("data-sec", "keys-stack");

    var labB = document.createElement("div");
    labB.className = "mg-drawer-stack-label";
    labB.id = "mg-drawer-stack-label-beats";
    labB.textContent = "Beats · staff · piano";
    stack.appendChild(labB);
    var beatsHost = document.createElement("div");
    beatsHost.id = "mg-drawer-beats-host";
    stack.appendChild(beatsHost);
    ...
    function doEmbed() {
      ...
      var mazeOk = false;
      var trailOk = false;
      /* Park QWERTY float keyboard if it was left open from Tools */
      try {
        if (window.__mgFloatKb) {
          if (window.__mgFloatKb.unembed) window.__mgFloatKb.unembed();
          if (window.__mgFloatKb.close) window.__mgFloatKb.close();
        }
      } catch (eK) {}
      try {
        /* never open() as float first — embed only (one surface) */
      ...
      if (!beatsOk) {
        beatsHost.innerHTML =
          '<p class="drw-hint">Beats piano missing — loading keyboard-beats…</p>';
      }
      if (!mazeOk) {
        mazeHost.innerHTML =
          '<p class="drw-hint">Maze missing — inject memory-maze-gsplat</p>';
      }
      setStatus(
        "Keys · " +
          (beatsOk ? "piano" : "—") +
      ...
      return beatsOk || mazeOk;
    }

    function loadKeysStack(done) {
      var left = 3;
      function tick() {
        left -= 1;
        if (left <= 0) {
      ...
      function need(key) {
        try {
          if (window.__mgLazy && window.__mgLazy.need) {
            window.__mgLazy.need(key, function () {
              tick();
            });
            return;
          }
    ...
    loadKeysStack();
    setTimeout(function () {
      if (!document.getElementById("mg-drawer-beats-host")) return;
      if (!window.__mgKeyboardBeats || !window.__mgMemoryMaze) loadKeysStack();
      else doEmbed();
    }, 400);
    setTimeout(function () {
      if (!document.getElementById("mg-drawer-beats-host")) return;
  ...
  function setOpen(on) {
    ...
    try {
      ...
      if (
        ...
        var rd = document.getElementById("mg-right-drawer");
        if (rd && rd.classList.contains("open")) {
          rd.style.setProperty("z-index", "2147483645", "important");
          rd.style.setProperty("right", "0", "important");
          rd.style.setProperty("left", "auto", "important");
        }
      }
    } catch (eDual) {}
    raiseEdges();
  ...
  function mount() {
    ...
    if (document.getElementById("mg-tools-drawer")) {
      try {
        var needRemount =
          ...
          !document.getElementById("mg-tools-drawer-css");
        /* New IIFE after hot-reload: local modeRail is null even if DOM exists */
        if (!modeRail || !el || !body) needRemount = true;
        ...
        if (needRemount) {
          ...
          /* rebind locals to live DOM (safe path) */
          el = document.getElementById("mg-tools-drawer");
          modeRail = document.getElementById("mg-tools-mode-rail");
          body =
            document.getElementById("mg-tools-body") ||
            (el && el.querySelector("#mg-tools-body"));
          statusEl =
            document.getElementById("mg-tools-status") ||
            (el && el.querySelector("#mg-tools-status"));
          ensureCss();
          paintModeRail();
          positionModeRail();
          raiseEdges();
    ...
    /* left docked via positionModeRail() — do not freeze left:0!important here */
